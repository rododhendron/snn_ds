...
begin
	...
	@variables vpre(t) = iparams.vrest wpre(t) = iparams.w0 Ipre(t) [input = true]
	@variables vpost(t) wpost(t) = iparams.w0 Ipost(t) g_syn(t)
	if experiment == "ampa"
		synapse_eq = D(g_syn) ~ exp(-g_syn / tau_GABAa_rise)
	elseif experiment == "gabaa"
		synapse_eq = D(g_syn) ~ exp(-g_syn / tau_GABAa_rise)
	end
	eqs = [
    	D(vpre) ~ (- Je * (vpre - vrest) + Je * delta * exp((vpre - vthr) / delta) - wpre - Ipre * (vpre - input_potential))/ Cm
    	D(wpre) ~ (-wpre + a * (vpre - vrest)) / TauW
		Ipre ~ step_fn(t, input_value)
    	D(vpost) ~ (- Je * (vpost - vrest) + Je * delta * exp((vpost - vthr) / delta) - wpost - Ipost ) / Cm #- Ipre * (vpre - input_potential) )/ Cm
    	D(wpost) ~ (-wpost + a * (vpost - vrest)) / TauW
		D(g_syn) ~ exp(-g_syn / tau_GABAa_rise)
		Ipost ~ g_syn * (vpost - EGABA)
	]
end
...
begin
	# overrides
	spike_affect_event_pre = [vpre ~ iparams.vrest, wpre ~ wpre + iparams.b * 1, g_syn ~ g_syn + 1]
	spike_affect_event_post = [vpost ~ iparams.vrest, wpost ~ wpost + iparams.b * 1]
	if experiment == "ampa"
		params = SLVector(iparams; input_value=1e-9)
	end
end
...
@named neuron = ODESystem(eqs, t; tspan=tspan, continuous_events=[
	[vpre ~ params.vthr + 50] => spike_affect_event_pre, [vpost ~ params.vthr + 50] => spike_affect_event_post
])
...
manifest_format = "2.0"
project_hash = "e9ece029b2035d925dc0766a0d5d495b6a9a214f"
